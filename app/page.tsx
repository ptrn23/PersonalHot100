    ...
    if (!isValidDateString(params.week)) {
      return (
        <div className="min-h-screen flex flex-col items-center justify-center p-10 bg-white text-center">
          ...
          <p className="text-gray-500 mb-8 font-medium">Dates must be requested in YYYY-MM-DD format.</p>
          <Link href="/" className="bg-black text-white px-6 py-3 rounded-lg font-bold uppercase text-sm hover:bg-gray-800 transition-colors">
      ...
      if (containingWeek) {
        return (
          <div className="min-h-screen flex flex-col items-center justify-center p-10 bg-gray-50 text-center">
            <h1 className="text-3xl font-black mb-4 uppercase tracking-tighter text-gray-800">Chart Not Found</h1>
            <p className="text-gray-600 mb-8 max-w-md">
              The date you entered falls under the chart week ending on <strong>{containingWeek.end_date}</strong>.
            </p>
            <Link href={`/?week=${containingWeek.end_date}`} className="bg-blue-600 text-white px-8 py-3 rounded-lg font-bold uppercase text-sm hover:bg-blue-700 transition-colors">
