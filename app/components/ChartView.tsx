...
import { Search, X, List, Grid as GridIcon, Square, Columns, Maximize, Download } from 'lucide-react';
...
export default function ChartView({ entries, availableWeeks, activeWeekDate, formattedDateRange }: Props) {
  ...
  const [exportChunk, setExportChunk] = useState<any[]>([]); 
    ...
    try {
      if (viewMode === 'list') {
        ...
        for (let i = 0; i < chunks.length; i++) {
          ...
          saveAs(dataUrl, `PH100_${activeWeekDate}_${startRank}-${endRank}.png`);
        }
      } else {
        ...
        <header className="mb-6 flex justify-between items-end px-2">
          ...
          <div className="flex items-center gap-4 text-right">
            <div className="relative flex items-center text-gray-400">
              ...
              />
              {searchQuery && (
                <button onClick={() => setSearchQuery('')} className="absolute right-2 text-gray-400 hover:text-[#B30000] transition-colors">
            ...
            <div className="flex items-center bg-gray-100 p-1 rounded-lg border border-gray-200 shadow-sm text-gray-400">
              <button title="List View" onClick={() => setViewMode('list')} className={`p-1.5 rounded-md transition-all ${viewMode === 'list' ? 'bg-white text-black shadow-sm' : 'hover:text-black'}`}>
                <List size={16} strokeWidth={2.5} />
              </button>
              <button title="Grid View" onClick={() => setViewMode('grid')} className={`p-1.5 rounded-md transition-all ${viewMode === 'grid' ? 'bg-white text-black shadow-sm' : 'hover:text-black'}`}>
            ...
            <div className="flex items-center bg-gray-100 p-1 rounded-lg border border-gray-200 shadow-sm text-gray-400">
              {WIDTH_MODES.map((mode) => (
                <button key={mode.id} title={`${mode.id.charAt(0).toUpperCase() + mode.id.slice(1)} Width`} onClick={() => setLayoutWidth(mode.id as 'slim' | 'normal' | 'wide')} className={`p-1.5 rounded-md transition-all ${layoutWidth === mode.id ? 'bg-white text-black shadow-sm' : 'hover:text-black'}`}>
                  {mode.icon}
                </button>
              ))}
            </div>
            ...
            <button 
              ...
              title={viewMode === 'list' ? "Export 4-Part Image Series" : "Export 10x10 Grid Image"}
            >
              {isExporting ? (
        ...
        {filteredEntries && filteredEntries.length > 0 ? (
          viewMode === 'list' ? (
            ...
            <div className={`grid gap-0 border-y-2 border-black transition-all duration-300 ease-in-out ${getGridCols()}`}>
              {filteredEntries.map((entry) => (
                <div key={entry.id} className="aspect-square bg-gray-100 overflow-hidden relative group">
                  {entry.songs?.albums?.cover_url ? (
                    <img src={entry.songs.albums.cover_url} alt={entry.songs.title} className="w-full h-full object-cover" loading="lazy" />
                  ) : (
                  ...
                  <div className="absolute top-0 left-0 bg-black text-white text-xs font-black px-2 py-0.5 shadow-sm">{entry.rank}</div>
                </div>
              ))}
            </div>
          )
      ...
      >
        <div ref={exportContainerRef} className="bg-white p-12 text-sm text-gray-900">
          ...
          <div className="flex justify-between items-end border-b-2 border-black pb-4 mb-4">
            <h1 className="text-3xl font-black uppercase tracking-tighter">Personal Charts</h1>
          ...
          {viewMode === 'list' ? (
            ...
            <div className="grid gap-0 border-y-2 border-black grid-cols-10">
              {exportChunk.map((entry) => (
                <div key={entry.id} className="aspect-square bg-gray-100 overflow-hidden relative group">
                  {entry.songs?.albums?.cover_url ? (
                    <img src={entry.songs.albums.cover_url} alt={entry.songs.title} className="w-full h-full object-cover" loading="eager" />
                  ) : (
                    <div className="w-full h-full flex flex-col items-center justify-center text-gray-400 font-bold uppercase text-[10px] bg-gray-50 border border-gray-200">
                      <span>No Cover</span>
                    </div>
