...
export const runCertifications = async (isFinalizing?: boolean, overrideTargetDate?: string) => {
  ...
  const albumIds = Array.from(
    new Set(weekEntries.map((e) => (e.songs as any)?.album_id).filter(Boolean)),
  );

  console.log(
    `Found ${songIds.length} unique songs and ${albumIds.length} unique albums to evaluate.`,
  );
  ...
  if (albumIds.length > 0) {
    certsQuery = certsQuery.or(
      `song_id.in.(${songIds.join(",")}),album_id.in.(${albumIds.join(",")})`,
    );
  } else {
  ...
  const { error: insertError } = await supabase.from("certifications").insert(certsToInsert);
