...
class PointsCalculator {
  ...
  static calculateWeightedPoints(currentPoints: number, prevPoints: number, twoWeeksAgoPoints: number): number {
    return Math.floor(currentPoints + Math.floor(prevPoints * 0.3) + Math.floor(twoWeeksAgoPoints * 0.2));
  }
}
...
async function generateWeeklyChart() {
  ...
  for (const scrobble of scrobbles) {
    ...
    if (!metricsMap.has(sid)) {
      metricsMap.set(sid, {
        ...
        streams: 0, sales: 0, airplay: 0, _currentStreak: 0
      });
    }
  ...
  let chart = Array.from(metricsMap.values());
  
  for (const entry of chart) {
    entry.current_week_points = PointsCalculator.calculateRawPoints(entry.streams, entry.sales, entry.airplay);
  ...
  const entriesToInsert = chart.map((entry, index) => {
    ...
    if (rank <= 5) {
      console.log(`#${rank}: ${entry.title} by ${entry.artist} | Pts: ${entry.total_points} (Streams: ${entry.streams}, Sales: ${entry.sales}, Airplay: ${entry.airplay})`);
    }
