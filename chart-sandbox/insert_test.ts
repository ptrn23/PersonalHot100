...
async function runTestInsertion() {
  ...
  for (const track of tracks) {
    ...
    const { data: week, error: weekErr } = await supabase
      .from("chart_weeks")
      .upsert(
        { start_date: weekBounds.startDate, end_date: weekBounds.endDate },
        { onConflict: "end_date" },
      )
      .select()
      .single();
    if (weekErr) {
      console.error("Week Upsert Error:", week);
      throw weekErr;
    }
    ...
    if (scrobbleErr && scrobbleErr.code === "23505") {
      ...
      successCount++;
      console.log(
        `✅ Saved: ${track.name} by ${track.artist["#text"]} (Week: ${weekBounds.startDate})`,
      );
    }
  }
