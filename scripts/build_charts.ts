...
const SPECIAL_MILESTONES: { [key: number]: string } = {
  52: "one year",
  104: "two years",
};
...
async function fetchCoverFromLastFM(
  artist: string,
  album: string,
  apiKey: string,
): Promise<string> {
...
function generateFeed(songs: SongEntry[]) {
  ...
  songs.forEach((song) => {
    ...
    if (song.status === "new") {
      song.feed!.push(
        `“${song.title}” by ${song.artist} debuts at #${song.rank} in ${CHART_NAME} Hot 100.`,
      );
    }
    ...
    // Re-entry
      ...
      if (song.isNewPeak) {
        song.feed!.push(
          `“${song.title}” by ${song.artist} reaches a new peak in ${CHART_NAME} Hot 100, reentering at #${song.rank}.`,
        );
      } else {
        song.feed!.push(
          `“${song.title}” by ${song.artist} reenters ${CHART_NAME} Hot 100 at #${song.rank}.`,
        );
      }
    }
    ...
    if (song.rank === 1 && song.lastWeek) {
      ...
      if (song.peakStreak === "1" || streak === 1) {
        if (song.peak === "1")
          song.feed!.push(
            `“${song.title}” by ${song.artist} reaches #1 in the ${CHART_NAME} Hot 100 for the first time.`,
          );
      } else if (song.lastWeek === 1) {
        song.feed!.push(
          `“${song.title}” by ${song.artist} spends a ${ordinal(streak)} week at #1 in the ${CHART_NAME} Hot 100.`,
        );
      } else {
        song.feed!.push(
          `“${song.title}” by ${song.artist} returns to #1 in the ${CHART_NAME} Hot 100 for a ${ordinal(streak)} nonconsecutive week at the top.`,
        );
      }
    }
    ...
    if (SPECIAL_MILESTONES[woc]) {
      ...
      song.feed!.push(
        `“${song.title}” by ${song.artist} spends its ${ordinal(woc)} week in ${CHART_NAME} Hot 100 this week.`,
      );
    }
    ...
    if (song.status === "rise" && song.isNewPeak) {
      song.feed!.push(
        `“${song.title}” by ${song.artist} reaches a new peak in ${CHART_NAME} Hot 100, rising ${song.change} spot${song.change !== 1 ? "s" : ""} to #${song.rank}.`,
      );
    }
    ...
    if (song.status === "rise" && song.lastWeek) {
      if (song.rank <= 5 && song.lastWeek > 5) {
        song.feed!.push(
          `“${song.title}” by ${song.artist} climbs inside the top 5 of ${CHART_NAME} Hot 100, rising ${song.change} spot${song.change !== 1 ? "s" : ""} to #${song.rank}.`,
        );
      } else if (song.rank <= 10 && song.lastWeek > 10) {
        song.feed!.push(
          `“${song.title}” by ${song.artist} climbs inside the top 10 of ${CHART_NAME} Hot 100, rising ${song.change} spot${song.change !== 1 ? "s" : ""} to #${song.rank}.`,
        );
      }
    }
  });
  ...
  // Biggest % Gainer
  const bigPctGainer = songs.reduce(
    (prev, curr) => (getPct(curr) > getPct(prev) ? curr : prev),
    songs[0],
  );
  if (getPct(bigPctGainer) > 0) {
    const pct = Math.round(getPct(bigPctGainer) * 100);
    bigPctGainer.feed!.push(
      `“${bigPctGainer.title}” by ${bigPctGainer.artist} is the biggest percentage gainer in ${CHART_NAME} Hot 100 this week, rising ${pct}% to ${Math.round(bigPctGainer.points)} points.`,
    );
  }

  // Biggest Position Gainer
  const bigPosGainer = songs.reduce(
    (prev, curr) => (getRise(curr) > getRise(prev) ? curr : prev),
    songs[0],
  );
  if (bigPosGainer.status === "rise") {
    bigPosGainer.feed!.push(
      `“${bigPosGainer.title}” by ${bigPosGainer.artist} is the biggest position gainer in ${CHART_NAME} Hot 100 this week, rising ${bigPosGainer.change} spot${bigPosGainer.change !== 1 ? "s" : ""} to #${bigPosGainer.rank}.`,
    );
  }

  // Biggest % Faller
  const bigPctFaller = songs.reduce(
    (prev, curr) => (getFallPct(curr) < getFallPct(prev) ? curr : prev),
    songs[0],
  );
  if (getFallPct(bigPctFaller) < 0 && getFallPct(bigPctFaller) !== Infinity) {
    const pct = Math.abs(Math.round(getFallPct(bigPctFaller) * 100));
    bigPctFaller.feed!.push(
      `“${bigPctFaller.title}” by ${bigPctFaller.artist} is the biggest percentage faller in ${CHART_NAME} Hot 100 this week, dropping ${pct}% to ${Math.round(bigPctFaller.points)} points.`,
    );
  }

  // Biggest Position Faller
  const bigPosFaller = songs.reduce(
    (prev, curr) => (getFall(curr) > getFall(prev) ? curr : prev),
    songs[0],
  );
  if (bigPosFaller.status === "fall") {
    bigPosFaller.feed!.push(
      `“${bigPosFaller.title}” by ${bigPosFaller.artist} drops ${bigPosFaller.change} spot${bigPosFaller.change !== 1 ? "s" : ""} to #${bigPosFaller.rank} — the biggest drop this week in ${CHART_NAME} Hot 100.`,
    );
  }
  ...
  if (songs.some((s) => s.isTopSales)) {
    ...
    topSalesSongs.forEach((s) => {
      s.feed!.push(
        `“${s.title}” by ${s.artist} is the #1 best selling song in ${CHART_NAME} Hot 100 this week with ${s.salesPoints.toLocaleString()} sales units.`,
      );
    });
  }
  ...
  if (songs.some((s) => s.isTopStreams)) {
    ...
    topStreamsSongs.forEach((s) => {
      s.feed!.push(
        `“${s.title}” by ${s.artist} is the #1 most streamed song in ${CHART_NAME} Hot 100 this week with ${s.streamsPoints.toLocaleString()} streams units.`,
      );
    });
  }
  ...
  if (songs.some((s) => s.isTopAirplay)) {
    ...
    topAirplaySongs.forEach((s) => {
      s.feed!.push(
        `“${s.title}” by ${s.artist} is the #1 most airplayed song in ${CHART_NAME} Hot 100 this week with ${s.airplayPoints.toLocaleString()} airplay units.`,
      );
    });
  }
  ...
  if (songs.some((s) => s.isTopUnits)) {
    ...
    topUnitsSongs.forEach((s) => {
      s.feed!.push(
        `“${s.title}” by ${s.artist} is the #1 overall units gainer in ${CHART_NAME} Hot 100 this week with ${s.units.toLocaleString()} total units.`,
      );
    });
  }
}
